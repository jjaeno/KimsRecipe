// Responsibility: 인증/회원가입 도메인 규칙을 처리한다. 입력 검증, 중복 체크, 비밀번호 해시, 토큰 발급 등 비즈니스 로직을 담당하며, SQL 호출은 Repository에 위임한다.
...
async function signUp(payload) {
  ...
  const userId = await userRepo.createUser({ username, name, password: hashedPassword, phone });
  return { userId };
}
